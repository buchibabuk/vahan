...
public class PaymentGatewayImplementation {
    ...
    private static final Logger LOGGER = Logger.getLogger(PaymentGatewayImplementation.class);
 ...
 * by passing off_cd and emp_cd from SessionVariables
 */
    public List<PaymentGatewayDobj> getApplicationNoAndAmountDetail(String paymentId, String purCd, String emp_cd,int off_cd) {
        ...
        try {
            ...
            ps = tmgr.prepareStatement(sql);
            ps.setLong(1, Long.parseLong(emp_cd));
            if (!paymentId.equals("New Cart")) {
                ps.setString(2, paymentId);
                ps.setInt(3, off_cd);
            } else {
 ...
 * author: Ramesh 
 * Changed HttpSession references their variable counterparts
 * by passing emp_cd from SessionVariables
 * if user want to revert back to Add To Cart Form then
 * that application number get deleted.
 */
     public String deleteApplicationNumber(String applicationNumber, TransactionManager tmgr,String emp_cd) throws VahanException {
        PreparedStatement ps = null;
        String message = "";
        String sql = "";
        try {
            ServerUtility.revertBackInOnlinePayment(tmgr, applicationNumber, emp_cd);
            message = "Employee Deleted Successfully!!!";
        } catch (Exception e) {
            LOGGER.error(e.toString() + " " + e.getStackTrace()[0]);
            throw new VahanException(e.getMessage());
    ...
    public Object[] updateTransactionNumber(TransactionManager tmgr, String stateCd, String applicationNumberList) throws VahanException {
        ...
        try {
            String paymentInward = ServerUtility.getUniqueOfficeRcptNo(tmgr, stateCd, Util.getSelectedSeat().getOff_cd(), TableConstants.DEALER_PAYMENTID_FLAG, false);
            ps = tmgr.prepareStatement(sql);
 ...
 * author: Ramesh 
 * Changed HttpSession references their variable counterparts
 * by passing off_cd from SessionVariables
 */
        public Object[] updateTransactionNumber(TransactionManager tmgr, String stateCd, String applicationNumberList, int off_cd, String emp_cd) throws VahanException {
        PreparedStatement ps = null;
        String sql = null;
        sql = "update vp_rcpt_cart set transaction_no = ? where user_cd = ? and transaction_no IS NULL and appl_no IN (" + applicationNumberList + ")";
        Object[] validate = new Object[2];
        try {
            String paymentInward = ServerUtility.getUniqueOfficeRcptNo(tmgr, stateCd, off_cd, TableConstants.DEALER_PAYMENTID_FLAG, false,emp_cd);
            ps = tmgr.prepareStatement(sql);
            if (paymentInward != null && !paymentInward.equals("")) {
                ps.setString(1, paymentInward);
            } else {
            ...
            ps.setLong(2, Long.parseLong(emp_cd));
            int updatedCount = ps.executeUpdate();
            if (updatedCount > 0) {
                validate[0] = updatedCount;
                validate[1] = paymentInward;
 ...
 * by passing emp_cd from SessionVariables
 */
    public String getPurCdList(String paymentId,String emp_cd) {
        String sql = null;
        PreparedStatement ps = null;
        TransactionManager tmgr = null;
        String purCdList = "";
 ...
 * author: Ramesh 
 * Changed HttpSession references their variable counterparts
 * by passing off_cd,state_cd,emp_cd,tmConfigDobj,clientIpAddress,selectedRoleCode and actionCode from SessionVariables
 */
    public void rollbackToPreviousActionCode(Status_dobj status_dobj, String appl_no,String emp_cd,int off_cd,String state_cd,TmConfigurationDobj tmConfigDobj,String clientIpAddress,String selectedRoleCode, int actionCode, String userCategory) throws VahanException {
        ...
        try {
            ...
            status_dobj = ServerUtility.webServiceForNextStage(status_dobj, tmgr, emp_cd, off_cd, state_cd);

            status_dobj.setOffice_remark("");
            status_dobj.setPublic_remark("");
            status_dobj.setCntr_id("");
            status_dobj.setEmp_cd(Long.parseLong(emp_cd));
            status_dobj.setOff_cd(off_cd);

//            ServerUtility.fileFlow(tmgr, status_dobj);

            ServerUtility.fileFlow(tmgr, status_dobj, actionCode, selectedRoleCode, state_cd, off_cd, emp_cd, userCategory, clientIpAddress,
            ...
            if (rs.next()) {
                if (rs.getInt("action_cd") == status_dobj.getAction_cd()) {
                    String deleteStatus = this.deleteApplicationNumber(appl_no, tmgr,emp_cd);
    ...
    public List<PaymentGatewayDobj> getFailCartList(Date fromDate, Date uptoDate, String stateCd, int offCd, String empCd) {
        ...
        try {
            ...
            sql = "  select f.moved_on,f.transaction_no , count(distinct f.appl_no) as totalAppl,array_to_string(array(select vph.appl_no from vph_rcpt_cart_fail vph "
                    ...
                    + " vph_rcpt_cart_fail f left outer join vahanpgi.vp_pgi_details pgi on f.transaction_no = pgi.payment_id where user_cd = ? and state_cd = ? and off_cd = ?  and f.moved_on between ? and (?::date + '1 day'::interval) and f.reason !~ 'no web' \n"
 ...
 */
    ...
    public String getTransactionNumber(String stateCd, String applicationNumberList,int off_cd,String emp_cd) throws VahanException {
        String paymentInward = "";
        TransactionManager tmgr = null;
        try {
            tmgr = new TransactionManager("getTransactionNumber()");
            Object[] validate = this.updateTransactionNumber(tmgr, stateCd, applicationNumberList,off_cd,emp_cd);
            Integer updatedCount = (Integer) validate[0];
            String transNo = (String) validate[1];
            if (updatedCount > 0 && transNo != null && !transNo.equals("")) {
                paymentInward = transNo;
