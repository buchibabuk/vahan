...
import static nic.vahan.form.impl.Receipt_Master_Impl.isDuplicateRcptNo;
 ...
 */
public class ReceiptMasterImplementation {
    

    private static final Logger LOGGER = Logger.getLogger(ReceiptMasterImplementation.class);
    ...
    /**
     * @author Kartikey Singh
     * 1. Changed reference of getTmConfiguration, need to check if this is correct
     * 2. Changed reference of ServerUtil.getUniqueOfficeRcptNo. Removed references to Util class inside.
     */
    public static String generateNewRcptNo(int off_cd, TransactionManager tmgr, String userStateCode, String empCode,
        ...
        PreparedStatement ps = null;
        RowSet rs = null;
        long userCode = Long.parseLong(empCode);
        boolean isRcptNoFound = false;
        try {
            TmConfigurationDobj tmConf = Utility.getTmConfiguration(null, userStateCode);
            if (tmConf.isAuto_cash_rcpt_gen()) {
                String rcpt = ServerUtility.getUniqueOfficeRcptNo(tmgr, userStateCode, off_cd, TableConstants.CASH_RCPT_FLAG, false, empCode);
                return rcpt;
            }
            ...
            if (!isRcptNoFound) {
                ...
                if (isDuplicateRcptNo(userStateCode, selectedOffCode, rcptno, tmgr)) {
                    throw new VahanException("Cash Receipt No already available, Please reset the Cash Receipt No Master properly...");
                }
            }
        } catch (SQLException e) {
