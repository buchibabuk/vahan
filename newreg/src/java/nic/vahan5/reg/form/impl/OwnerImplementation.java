...
public class OwnerImplementation {
    ...
    private static final Logger LOGGER = Logger.getLogger(OwnerImplementation.class);
     ...
     * @author Kartikey Singh
     */
    public Owner_dobj getValidateVtOwnerTempDobj(String chasi_no, String userStateCode, int offCode, boolean cnginfo_from_cngmaker) throws VahanException {
        Owner_dobj owner_dobj = null;
        TransactionManager tmgr = null;
        PreparedStatement ps = null;
        int offTo = 0;
        ...
        try {
            ...
            if (rs.next() && rs.getBoolean("v4_started")) {
                ...
                if (stateTo.equals(userStateCode) && (offTo == offCode)
                        || (rs.getString("state_cd").equals(userStateCode) && rs.getInt("off_cd") == offCode && rs.getString("purpose").equalsIgnoreCase("BB"))) {
                    rs.beforeFirst();
                    owner_dobj = fillFrom_vt_owner_temp_with_state_cd_off_cd(rs);
                    owner_dobj.setRegn_dt(new Date());
                    TempRegDobj tempDobj = new TempRegDobj();
                    ...
                    owner_dobj.setLinkedRegnNo(getLinkedVehDetails(rs.getString("appl_no"), tmgr));
                    //Insurance
                    InsDobj insDobj = InsImplementation.set_ins_dtls_db_to_dobj(owner_dobj.getRegn_no(), null, rs.getString("state_cd"), rs.getInt("off_cd"));
                    owner_dobj.setInsDobj(insDobj);

                    //Axel Details
                    AxleDetailsDobj axleDobj = AxleImplementation.setAxleVehDetails_db_to_dobj(null, owner_dobj.getRegn_no(), rs.getString("state_cd"), rs.getInt("off_cd"));
                    owner_dobj.setAxleDobj(axleDobj);

                    //RetroFitting Details
                    RetroFittingDetailsDobj retroDobj = RetroFittingDetailsImplementation.setCngDetails_db_to_dobj_VT(owner_dobj.getRegn_no(), rs.getString("state_cd"), rs.getInt("off_cd"), cnginfo_from_cngmaker);
                    owner_dobj.setCng_dobj(retroDobj);

                    //HPA Details
                    List<HpaDobj> listHpaDobj = new ArrayList<>();
                    HpaDobj hpaDobj = new HpaImplementation().set_HPA_appl_db_to_dobj(null, owner_dobj.getRegn_no(), 0, rs.getString("state_cd"), rs.getInt("off_cd"));
                    listHpaDobj.add(hpaDobj);
                    owner_dobj.setListHpaDobj(listHpaDobj);

                    //Imported Vehicle Details
                    ImportedVehicleDobj impDobj = ImportedVehicleImplementation.setImpVehDetails_db_to_dobj(null, owner_dobj.getRegn_no(), rs.getString("state_cd"), rs.getInt("off_cd"));
                    owner_dobj.setImp_Dobj(impDobj);

                } else {
                    if (purpose != null && !purpose.equalsIgnoreCase("OM")) {
                        String offName = ServerUtility.getOfficeName(offTo, stateTo);
                        stateTo = MasterTableFiller.getStateDescrByStateCode(stateTo);
                        throw new VahanException("Vehicle has Taken Temporary Registration [" + temp_regn_no + "] for State[" + stateTo + "] and Office[" + offName + "(" + offTo + ")], Issue Date [" + issue_date + "],"
                                + " Please Register this Vehicle in State[" + stateTo + "] at Office[" + offName + "(" + offTo + ")]");
                    }
    ...
    public Owner_dobj getValidateVtOwnerTempDobj(String chasi_no, String userStateCode, int offCode) throws VahanException {
        Owner_dobj owner_dobj = null;
        TransactionManager tmgr = null;
        PreparedStatement ps = null;
        int offTo = 0;
     ...
     * @author Kartikey SIngh
     */
    public Owner_dobj set_Owner_appl_db_to_dobj(String regn_no, String appl_no, String chasi_no, int pur_cd,
            String stateCode, int selectedOffCode) throws Exception {
        TransactionManager tmgr = null;
        PreparedStatement ps = null;
        Owner_dobj owner_dobj = null;
        RowSet rs = null;
        ...
        try {
            ...
            if (TableConstants.VM_TRANSACTION_MAST_NEW_VEHICLE == pur_cd
                ...
                if (appl_no != null && !appl_no.equalsIgnoreCase("")) {
                    ...
                    ps.setString(1, parameterValue);
                    rs = tmgr.fetchDetachedRowSet_No_release();
                    owner_dobj = fillFrom_Va_OwnerDobj(rs, stateCode);
                ...
                if (appl_no != null && !appl_no.equalsIgnoreCase("")) {
                    ...
                    if (owner_dobj == null) {
                        ...
                        ps.setString(1, parameterValue);
                        rs = tmgr.fetchDetachedRowSet_No_release();
                        owner_dobj = fillFrom_vt_owner_temp(rs, stateCode, selectedOffCode);
                    }
     ...
     * @author Kartikey Singh
     */    
    public Owner_dobj set_Owner_appl_db_to_dobj_with_state_off_cd(String regn_no, String appl_no, String chasi_no, int pur_cd,
            String stateCode, int selectedOffCode) throws Exception {
        ...
        try {
            ...
            if (TableConstants.VM_TRANSACTION_MAST_NEW_VEHICLE == pur_cd
                ...
                if (regn_no != null) {
                    //add Top
                    int off_cd = selectedOffCode;
                    TmConfigurationDobj tm_Cong = Utility.getTmConfiguration(null, stateCode);
                    boolean allow_fitness_all_rto = tm_Cong.isAllow_fitness_all_RTO();
                    if (allow_fitness_all_rto) {
                        query = "select off_cd from  " + TableList.VT_OWNER + " where regn_no=? and state_cd = ?";
                        ps = tmgr.prepareStatement(query);
     ...
     * @author Kartikey Singh
     */
    public Owner_dobj fillFrom_vt_owner_temp(RowSet rs, String stateCode, int selectedOffCode) throws SQLException {
        Owner_dobj dobj = null;
        if (rs.next()) {
            dobj = new Owner_dobj();
            dobj.setState_cd(rs.getString("state_cd"));
            ...
            try {
                ...
                ReflectiveTapeDobj refDobj = new FitnessImplementation().getVtReflectiveTapeDobj(dobj.getRegn_no(), stateCode, selectedOffCode, tmgrOwner);
                dobj.setReflectiveTapeDobj(refDobj);
    ...
    public Owner_dobj fillFrom_Va_OwnerDobj(RowSet rsOwner) throws SQLException {
        ...
        try {
            if (rsOwner.next()) // found
            {
                ...
                ExArmyDobj exArmy = ExArmyImplementation.setExArmyDetails_db_to_dobj(rsOwner.getString("appl_no"), null, null, 0);
                owner_dobj.setExArmy_dobj(exArmy);

                InsDobj insDobj = InsImplementation.set_ins_dtls_db_to_dobj(null, rsOwner.getString("appl_no"), null, 0);
                owner_dobj.setInsDobj(insDobj);

                AxleDetailsDobj axleDetailsDobj = AxleImplementation.setAxleVehDetails_db_to_dobj(rsOwner.getString("appl_no"), null, null, 0);
                ...
                if ("DL".equals(owner_dobj.getState_cd()) && owner_dobj.getVh_class() == TableConstants.ERICKSHAW_VCH_CLASS) {
                    PendencyBankDobj dobj = new PendencyBankDetailImplementation().getBankSubsidyData(rsOwner.getString("appl_no"), rsOwner.getString("state_cd"), rsOwner.getInt("off_cd"));
                    if (dobj != null) {
    ...
    public Owner_dobj fillFrom_Va_OwnerDobj(RowSet rsOwner, String stateCode) throws SQLException {
        Owner_dobj owner_dobj = null;
        TransactionManager tmgrOwner = null;
        RowSet rsOwnerId = null;
        try {
            if (rsOwner.next()) // found
            {
                ...
                if (stateCode != null && stateCode.equals("RJ")) {
                    String reqdTaxMode = owner_dobj.getRqrd_tax_modes();
                    if (reqdTaxMode != null) {
                        String taxMode = ServerUtil.getTaxModeFromReqdTaxMode(reqdTaxMode);
                        owner_dobj.setTax_mode(taxMode);
     ...
     * @Comment No changes to be done, method is fit for REST usage
     */
    public Owner_dobj fillFrom_VT_OwnerDobj(RowSet rsOwner) throws SQLException, VahanException {
        ...
        try {
            if (rsOwner.next()) // found
            {
                ...
                ReflectiveTapeDobj refDobj = new FitnessImplementation().getVtReflectiveTapeDobj(owner_dobj.getRegn_no(), owner_dobj.getState_cd(), owner_dobj.getOff_cd(), tmgrOwner);
                owner_dobj.setReflectiveTapeDobj(refDobj);

                AxleDetailsDobj axleDetailsDobj = AxleImplementation.setAxleVehDetails_db_to_dobj(null, owner_dobj.getRegn_no(), owner_dobj.getState_cd(), owner_dobj.getOff_cd());
                if (axleDetailsDobj != null) {
                    owner_dobj.setAxleDobj(axleDetailsDobj);
                    owner_dobj.setNumberOfTyres(axleDetailsDobj.getTf_Front_tyre() + axleDetailsDobj.getTf_Other_tyre() + axleDetailsDobj.getTf_Rear_tyre() + axleDetailsDobj.getTf_Tandem_tyre());
                }
                InsDobj insDobj = InsImplementation.set_ins_dtls_db_to_dobj(owner_dobj.getRegn_no(), null, owner_dobj.getState_cd(), owner_dobj.getOff_cd());
                if (insDobj != null) {
                    owner_dobj.setInsDobj(insDobj);
                }
                // auction Details
     ...
     * @author Kartikey Singh
     */
    public static Map getDealerDetail(long userCode, String stateCode, int selectedOffCode) throws VahanException {
        ...
        try {
            String stateCd = stateCode;
            if (stateCd == null || selectedOffCode < 1) {
                throw new VahanException(TableConstants.SomthingWentWrong);
            }
            int offCd = selectedOffCode;
            tmgr = new TransactionManager("getMakerAndDealerCode");

            if (ServerUtility.validateDealerUserForAllOffice(userCode, stateCode)) {
                sql = "select m.dealer_name , m.dealer_cd \n"
                        + "from  vm_dealer_mast m left outer join tm_user_permissions up\n"
                        + "on m.dealer_cd = up.dealer_cd \n"
                        + "where m.state_cd = ? and up.user_cd = ? ";
     ...
     *
     * @author Kartikey Singh
     * Equated Util.getUserOffCode() and off_cd
     * Equated state_cd and Util.getUserStateCode()
     */
    public OwnerDetailsDobj getOwnerDetails(String regnNo, String state_cd, String off_cd,
            boolean allowFitnessAllRto) throws VahanException {
        OwnerDetailsDobj dobj = null;
        String sql = " SELECT owner.*,type.descr as regn_type_descr,owcode.descr as owner_cd_descr "
                + " FROM " + TableList.VIEW_VV_OWNER + " owner "
                + " left join vm_regn_type type on owner.regn_type=type.regn_typecode "
        ...
        try {
            ...
            tmgr = new TransactionManager("getOwnerDetails");
//            TmConfigurationDobj tm_Cong = Util.getTmConfiguration();
//            boolean allow_fitness_all_rto = allowFitnessAllRto;

            if (allowFitnessAllRto && !(off_cd.split(",").length > 1)) {
                String query = "select off_cd from  " + TableList.VT_OWNER + " where regn_no=? and state_cd = ?";
                ps = tmgr.prepareStatement(query);
                ps.setString(1, regnNo);
                ps.setString(2, state_cd);
...
//                }
     ...
     * @author Kartikey Singh
     */
    public OwnerDetailsDobj getVaOwnerDetails(String applNo, String stateCd, int offCd, String userCategory, boolean allowFitnessAllRTO) throws VahanException {
        OwnerDetailsDobj dobj = null;
        TransactionManager tmgr = null;
        PreparedStatement ps = null;
        try {
            ...
            if (!userCategory.equalsIgnoreCase(TableConstants.USER_CATG_FITNESS_CENTER_USER) && !allowFitnessAllRTO) {
                where_condition = " and off_cd=?";
            }
            ...
            if (rs.next()) {
                ...
                if (ServerUtility.isTransport(dobj.getVh_class(), null)) {
                    dobj.setVehTypeDescr(TableConstants.VM_VEHTYPE_TRANSPORT_DESCR);
                    dobj.setVehType(TableConstants.VM_VEHTYPE_TRANSPORT);
                    dobj.setVch_type(TableConstants.VM_VEHTYPE_TRANSPORT_DESCR);
                } else {
    ...
    public static void main(String[] args) {
        System.out.println(OwnerImplementation.cal(OwnerImplementation.cal(OwnerImplementation.cal(23, -32), 23), 8));
    }
...
//vt_owner_temp fill ownerDetialsDobj Niraj
    ...
    /*
        @author Kartikey Singh
    */
    public Owner_dobj set_Owner_appl_db_to_dobj_with_state_off_cd(String regn_no, String appl_no, String chasi_no, int pur_cd, TransactionManager tmgr,
            String stateCode, int selectedOffCode, boolean allowFitnessAllRto) throws Exception {
