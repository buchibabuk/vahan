 ...
 */
package nic.vahan5.reg.form.impl.permit;
...
import nic.vahan.form.impl.permit.PermitAuthCompleteImpl;
 ...
 */
public class PassengerPermitDetailImplementation {

    private static final Logger LOGGER = Logger.getLogger(nic.vahan.form.impl.permit.PassengerPermitDetailImpl.class);
    public Map<String, String> confige;

    /*
     * @author KArtikey Singh
     * Created this contructor to populate the configs instance variable whenever we create an object of this class
     */
    public PassengerPermitDetailImplementation(String stateCode) {
        confige = CommonPermitPrintImpl.getVmPermitStateConfiguration(stateCode);
    }
     ...
     * @author Kartikey Singh
     */
    public void insertVHA_PermitandUpdateVa_Permit(TransactionManager tmgr, PassengerPermitDetailDobj dobj, String appl_no, String role_cd,
            String userCategory, String empCode, String stateCode, int selectedOffCode) throws VahanException {
        try {
            String query;
            PreparedStatement ps;
            query = "SELECT * FROM " + TableList.VA_PERMIT + " where appl_no=? ";
            ...
            if (rs.next()) {
                ...
                if (dobj != null) {
                    ...
                    ps.setString(i++, dobj.getParking());
                    ps.setString(i++, dobj.getOffer_no());
                    if ((userCategory.equalsIgnoreCase("A") && !CommonUtils.isNullOrBlank(dobj.getOrder_no()))
                            || (!CommonUtils.isNullOrBlank(role_cd) && role_cd.equalsIgnoreCase(String.valueOf(TableConstants.TM_ROLE_PMT_APPLICATION_APPROVAL)) && !CommonUtils.isNullOrBlank(dobj.getOrder_no()))) {
                        ps.setString(i++, dobj.getOrder_no().toUpperCase());
                    } else {
                        ps.setString(i++, ServerUtil.getUniquePermitNo(tmgr, stateCode, selectedOffCode, 0, 0, "A"));
                    }
                    ps.setString(i++, dobj.getOrder_by());
                    if (dobj.getAppDisboolenValue() == null) {
                        ps.setDate(i++, null);
     ...
     * @author Kartikey Singh
     */
    public void insertVHA_Owner_PermitandUpdateVa_Owner_Permit(TransactionManager tmgr, PermitOwnerDetailDobj OwnerDobj, String appl_no, String empCode) throws SQLException, VahanException {
        String query;
        PreparedStatement ps;
        query = "SELECT * FROM " + TableList.VA_PERMIT_OWNER + " where appl_no=? ";
        ps = tmgr.prepareStatement(query);
     ...
     * @author Kartikey Singh
     */
    public void insertVaToVhaMultiAppAllow(TransactionManager tmgr, PermitOwnerDetailDobj OwnerDobj, String appl_no, String empCode) throws SQLException {
        String query;
        PreparedStatement ps;
        query = "SELECT * FROM " + TableList.VA_PMT_MULTI_APP_ALLOW + " where appl_no=? ";
        ps = tmgr.prepareStatement(query);
                        ...
                        if (rs3.next()) {
                            ...
                            try {
                                ...
                                if (rs4.next()) {
                                    ...
                                    if (ServerUtil.dateRange(authDobj.getAuthFrom(), 1, 0, -1).compareTo(validUpto) <= 0) {
...
//    }
    /*
     * @author Kartikey Singh
     * Commented the original code as both of them had the same signature
     * The only difference is that one line having Util.getUserStateCode() has been replaced
     * with the parameter stateCd
     */
    public PassengerPermitDetailDobj getPermitHistory(String regnNo, String stateCd) {
        TransactionManager tmgr = null;
        PassengerPermitDetailDobj permit_dobj = null;
