...
import static nic.vahan5.reg.server.ServerUtility.insertIntoVhaChangedData;
import static nic.vahan5.reg.server.ServerUtility.verifyForSmartCard;
        ...
        try {
            ...
            if (status_dobj.getCurrent_role() == TableConstants.TM_ROLE_APPROVAL
                ...
                if (noc_dobj.getPur_cd() == TableConstants.VM_TRANSACTION_MAST_NOC_CANCEL) {
                    ...
                    ApplicationInwardImplementation inwardImpl = new ApplicationInwardImplementation();
...
//Code Ends
    ...
    public void checkForNocVerificationAndEndorsement(Owner_dobj owner_dobj, String regnNo) throws VahanException, Exception {
        ...
        if (noc_dobj != null) {
            if (noc_dobj.getVt_owner_status().equals("N")) {
                Date vahan4StartDate = ServerUtility.getVahan4StartDate(noc_dobj.getState_to(), noc_dobj.getOff_to());
    ...
    public void checkForNocVerificationAndEndorsement(Owner_dobj owner_dobj, String regnNo, String userStateCode, int offCode) throws VahanException, Exception {
        NocDobj noc_dobj = ServerUtility.getChasiNoExist(owner_dobj.getChasi_no());
        if (noc_dobj != null) {
            if (noc_dobj.getVt_owner_status().equals("N")) {
                Date vahan4StartDate = ServerUtility.getVahan4StartDate(noc_dobj.getState_to(), noc_dobj.getOff_to());
                if (vahan4StartDate == null) {
                    if (noc_dobj.getState_cd().equals(userStateCode) && offCode == noc_dobj.getOff_cd() || noc_dobj.getState_to().equals(userStateCode) && offCode == noc_dobj.getOff_to()) {
                        ...
                        NocDobj nocVerifiedData = ServerUtility.getNocVerifiedData(regnNo, owner_dobj.getChasi_no());
                        if (nocVerifiedData == null) {
                            throw new VahanException(TableConstants.NOC_VERIFICATION);
                        }
                    }
