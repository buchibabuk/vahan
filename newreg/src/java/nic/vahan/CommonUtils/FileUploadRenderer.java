...
public class FileUploadRenderer extends CoreRenderer {
    ...
    public void decode(FacesContext context, UIComponent component) {
        ...
        if (!fileUpload.isDisabled()) {
            ...
            PrimeApplicationContext cc = PrimeRequestContext.getCurrentInstance().getApplicationContext();

            String uploader = cc.getConfig().getUploader();
            // boolean isAtLeastJSF22 = cc.isAtLeastJSF22();
            boolean isAtLeastJSF22 = cc.getEnvironment().isAtLeastJsf22();

            if (uploader.equals("auto")) {
                if (isAtLeastJSF22) {
                    if (isMultiPartRequest(context)) {
//                        NativeFileUploadDecoder.decode(context, fileUpload);
                        NativeFileUploadDecoder.decode(context, fileUpload, "fileupload");
                    }
                } else {
                    CommonsFileUploadDecoder.decode(context, fileUpload, "fileupload");
                }
